...
import com.venky.model.OrderItem;
import com.venky.model.User;
import com.venky.payload.OrderDto;
import com.venky.payload.OrderRequest;
import com.venky.payload.OrderResponse;
import com.venky.repository.CartRepository;
import com.venky.repository.OrderRepository;
...
public class OrderServiceImp {
	...
	public OrderDto orderCreate(OrderRequest request, String username) {
		...
		// find cart
		Cart cart = this.cartRepo.findById(cartId).orElseThrow(() -> new Resourses_Not_Found("Cart Not Found"));
		// getting CartItem
		...
		if (order.getOrderAmt() > 0) {
			...
			throw new Resourses_Not_Found("Plz Add Cart First and place Order");
		}
	...
	public void CancelOrder(int orderId) {
		Order order = this.orderReop.findById(orderId).orElseThrow(() -> new Resourses_Not_Found("Order not Found"));
